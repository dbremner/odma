		...
		LPSTR lpszDocId)
{
	...
	LogParNumber("odmHandle", reinterpret_cast<ULONG_PTR>(odmHandle));
	...
	if(odmHandle)
	{
		auto *pClient = static_cast<ODMClient *>(odmHandle);
		...
		DWORD activeTime, DWORD pagesPrinted, LPVOID sessionData, WORD dataLen)
{
	...
	LogParNumber("sessionData address", reinterpret_cast<ULONG_PTR>(sessionData));
		...
		LPMONIKER *ppMoniker)
{
	...
	LogString("\tOutput parameters:\r\n");
	LogParNumber("ppMoniker address", reinterpret_cast<ULONG_PTR>(ppMoniker));
	LogParNumber("Return value", odm);
		...
		REFIID riid, LPVOID *ppvObj)
{
	...
	LogParNumber("odmHandle", reinterpret_cast<ULONG_PTR>(odmHandle));
	LogParString("lpszDocId", lpszDocId);
	LogParNumber("riid address", reinterpret_cast<ULONG_PTR>(&riid));
	...
	LogString("\tOutput parameters:\r\n");
	LogParNumber("ppvObj address", reinterpret_cast<ULONG_PTR>(ppvObj));
	LogParNumber("Return value", static_cast<ULONG>(hRes));
		...
		WORD version, LPSTR lpszAppId, DWORD dwEnvData, LPVOID /*pReserved*/)
{
	...
	*pOdmHandle = nullptr;
	...
	ODMSTATUS odm = Registry.GetDMS(lpszAppId, dms_id);
	if(odm == ODM_SUCCESS) 
	{
	...
	LogString("\tOutput parameters:\r\n");
	LogParNumber("odmHandle", reinterpret_cast<ULONG_PTR>(*pOdmHandle));
	LogParNumber("Return value", odm);
		...
		LPVOID pInstanceData)
{
	...
	LogParString("lpszDocId", lpszDocId);
	LogParString("lpszFormat", lpszFormat);
	LogParNumber("pcbCallBack address", reinterpret_cast<ULONG_PTR>(pcbCallBack));
	LogParNumber("pInstanceData address", reinterpret_cast<ULONG_PTR>(pInstanceData));
			...
			LPVOID sessionData, WORD dataLen)
{
	...
	LogString("\tInput parameters:\r\n");
	LogParNumber("Handle", reinterpret_cast<ULONG_PTR>(odmHandle));
	LogParString("lpszDocId", lpszDocId);
	LogParNumber("pdwFlags", *pdwFlags);
	LogParNumber("activeTime", activeTime);
	LogParNumber("pagesPrinted", pagesPrinted);
			...
			DWORD flags, DWORD event, LPVOID lpData, DWORD dwDataLen, LPSTR lpszComment)
{
	...
	LogParNumber("lpData", reinterpret_cast<ULONG_PTR>(lpData));
